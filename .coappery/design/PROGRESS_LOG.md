...
- **家庭聚會 = 行動中心（有目的）**：家庭圈＝畀祝福（情感）；家庭聚會＝採取行動。**首頁不列商戶目錄**（只有：即將到來提醒／快速安排 tiles／我的聚會）；商戶只在「訂餐廳／訂蛋糕／買禮物／送花」嗰刻以**可篩選清單**出現（類型／標籤／地區／搜尋／排序，贊助標示）。需要→商戶匹配**標籤優先、主分類後備**。
...
## 九、家庭聚會重新定位：首頁＝行動中心（2026-09-10 追加）

**問題**：第八節版本把商戶列表直接放喺家庭聚會首頁 → 變成第二個「我的推薦」，同「有目的才需要商戶」嘅商業模式相反（用戶無目的被推商戶、商戶攞唔到真正有意向嘅客）。

**新定位（product_decisions v1.10 / family_gather.md v1.4）**：家庭圈＝畀祝福（情感）；家庭聚會＝採取行動（有目的）。

### A. 新資訊架構（`src/pages/FamilyGather.tsx`，175 行）
1. **即將到來**：`GET /api/reminders`（30 日內）→ 生日／節日／自訂重要日子 → 一撳「去安排」（帶場合／對象／日期）；**忌辰 → 「獻上思念」**（莊重分支）。
2. **需要安排什麼？** tiles：發起聚會 ／ 訂餐廳 ／ 訂蛋糕 ／ 買禮物 ／ 定日子。
3. **我的聚會**（`GatherList`，只列清單）。
   → **首頁完全冇商戶列表**。

### B. 商戶只在「需要」嗰刻出現
- `GatherQuickStart`（新）：撳 tile → 問「加入哪個聚會／建立新安排」→ 跳 `#/gather/:id?add=<kind>`（聚會詳情自動開「加入候選」）。
- `MerchantPicker`（新）：**搜尋 ＋ 類型（主分類）＋ 標籤 ＋ 區域／地區 ＋ 排序（推薦／按地區／按名稱）＋ 贊助標示**；忌辰模式先剔除 `ad_tier > 0`（零廣告）。
- `GatherAddOption`：改為「**先揀商戶 → 再填日期／時間／取貨／負責人**」。
- `GET /api/merchants-meta`（新）：只回傳「至少有一個已上架商戶」嘅地區／類型／標籤選項。
- 需要 → 商戶匹配：**標籤優先、主分類後備**（`src/utils/merchantQuery.ts`）—— 例：茶餐廳唔會出現喺「訂蛋糕」；完全冇標籤嘅舊商戶才用主分類後備。
- B4 推薦卡 CTA 改為 `#/family-gather?plan=1&occasion=festival`（行動，而非場景瀏覽）。

### C. 移除嘅舊碼（死碼清理）
- 刪 `src/utils/gatherScenes.ts` 同 `scripts/test-gather-scenes.mjs`（首頁唔再按場合列商戶）→ 由 `merchantQuery` 取代。
- 清走 8 個死 locale key（`scene_*`／`section_list`／`section_sponsored`／`empty`）；現 `gather.*` 108 個 key 全部有用（以腳本核對）。
