...
## 23. 忌辰／莊重場合零廣告憲章（2026-09-10 新增）

凡涉及**忌辰、葬禮、拜祭**等莊重場合嘅畫面（例：家庭聚會 `scene=memorial`、事件詳情忌辰版），一律套用**硬攔截**，先於任何廣告／排序邏輯執行：

- **零廣告、零贊助位**：不得出現任何付費商戶位（`ad_tier > 0` 一律排除），只可列**自然排序**（`ad_tier = 0`）且與場合相符之商戶（鮮花／拜祭）。
- **零商業 CTA**：不得出現聚會發起、投票、訂購、抽獎、優惠券、「去安排」等慶祝／商業按鈕；只可保留莊重動作（獻上思念／送上鮮花／翻睇相簿）。
- **視覺莊重**：不得使用綠色 accent／`green-glow`／慶祝 emoji；沿用 `bg-solemn` 系（承細步 3h 忌辰版規格）。
- **不可被參數繞過**：此規則唔可以由 URL、設定或商戶資料開關；新增任何場合畫面時，實作者有責任先確認屬否莊重場合，並於程式碼註明。
- 相關文件：`family_gather.md` §7、`product_decisions.md` v1.8。
