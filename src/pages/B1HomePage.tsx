...
/* ── SVG Icons（B1 專用，rules.md 第16條例外）── */
...
/* ── Top Bar Right Slot（純 icon，rules.md 第16條）── */
...
function TopBarRightSlot() {
  ...
  const focusOn = (e: React.FocusEvent<HTMLButtonElement>) => { e.currentTarget.style.outline = '3px solid var(--color-primary)'; e.currentTarget.style.outlineOffset = '2px' }
  const focusOff = (e: React.FocusEvent<HTMLButtonElement>) => { e.currentTarget.style.outline = 'none' }
  return (
    <>
      <button aria-label={t('top_bar.add_member')} style={btnStyle} onFocus={focusOn} onBlur={focusOff}><IconAddMember size={22} /></button>
      <button aria-label={t('top_bar.share')} style={btnStyle} onFocus={focusOn} onBlur={focusOff}><IconShare size={22} /></button>
      <button aria-label={t('top_bar.notifications')} style={btnStyle} onFocus={focusOn} onBlur={focusOff}>
...
export default function B1HomePage() {
  ...
  // mockup placeholder 外部圖 URL，正式版須替換為使用者實際上載圖片
  const AVATAR_SELF         = 'https://randomuser.me/api/portraits/men/72.jpg'
  const AVATAR_SPOUSE       = 'https://randomuser.me/api/portraits/women/68.jpg'
  const AVATAR_SON          = 'https://randomuser.me/api/portraits/men/32.jpg'
  const AVATAR_DIL          = 'https://randomuser.me/api/portraits/women/44.jpg'
  const AVATAR_GRANDSON     = 'https://randomuser.me/api/portraits/men/85.jpg'
  const AVATAR_GRANDDAUGHTER = 'https://randomuser.me/api/portraits/women/90.jpg'
    ...
    <div style={{ minHeight: '100svh', backgroundColor: 'var(--color-bg)', display: 'flex', flexDirection: 'column' }}>
      ...
      <main role="main" aria-label={t('app_name')} style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', paddingTop: '56px', paddingBottom: '80px', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        ...
        <section aria-label={t('gen2.layer_label')} style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', boxSizing: 'border-box' }}>
          ...
          <GenCarousel
            ariaLabelKey="gen2.layer_label"
            focusedPrimary={gen2FocPrimary}
            focusedSecondary={gen2FocSecondary}
            ...
            leftPeekPrimary={gen2PeekLeft}
            rightPeekPrimary={gen2PeekRight}
            dotsTotal={3}
            dotsActive={0}
          />
