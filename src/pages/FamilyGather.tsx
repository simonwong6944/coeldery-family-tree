/**
 * FamilyGather — 家庭聚會 placeholder 頁
 * 路由：#/family-gather  規格：細步 3e
 */
...
export default function FamilyGather() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100svh', backgroundColor: 'var(--color-bg)' }}>
      <TopBar titleKey="placeholder.family_gather_title" onBack={() => { window.location.hash = '#/' }} />
      <main
        style={{
          flex: 1,
          display: 'flex',
      ...
      >
        ...
        <p style={{ margin: 0, fontSize: '18px', color: 'var(--color-text-secondary)', lineHeight: 1.6 }}>
          {t('placeholder.family_gather_msg')}
        </p>
      </main>
      <BottomTabBar current="family_gathering" onTabChange={handleTabChange} />
    </div>
  )
}
